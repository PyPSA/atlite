...
cutout = atlite.Cutout(
    "cutout", bounds=de.buffer(1).bounds, time="2013-01", module="era5", dx=0.5, dy=0.5
)
...
cutout.grid[["x", "y"]].plot.scatter(
    x="x", y="y", ax=ax, transform=plate(), color=color, s=1, alpha=1
)
...
ax.axis("off")
ax.set_title(
    r"\begin{center}\textbf{1. Create Cutout} \\(Select spatio-temporal bounds)\end{center}",
    **title_params
)
...
ax.set_title(
    r"\begin{center}\textbf{2. Prepare Cutout} \\(Retrieve data per weather cell)\end{center}",
    **title_params
)
...
ax.text(x=0.5, y=0.8, s=r"$\cdot$ \\$\cdot$ \\ $\cdot$", ha="center", fontsize=30)
ax.axis("off")
...
ds, capacity = cutout.wind(
    turbine, shapes=regions, layout=layout, return_capacity=True, per_unit=True
)


ax = fig.add_subplot(gs[3:, :3])
ax.set_title(
    r"\begin{center}\textbf{3. Convert Cutout} \\ (Calculate potentials and timeseries per region)\end{center}",
    **title_params
)
# ax.set_title(r"\textbf{3. Convert Cutout}", **title_params)
regions.plot(
    ax=ax,
    column=capacity.to_series() / 1e3,
    legend=True,
    cmap="Greens",
    legend_kwds=dict(label="Capcity potential [GW]", location="right", shrink=0.8),
)

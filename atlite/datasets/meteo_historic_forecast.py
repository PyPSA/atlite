...
"""
Module for downloading and curating historic forecast data from different weather prediction models (via Open-Meteo).
...
# Define data source URLs (easy to switch if needed)
...
requirements = {'x': slice(-90, 90, 0.1),
                'y': slice(-90, 90, 0.1),
                'offset': (pd.Timestamp('2022-01-01')-pd.Timestamp.utcnow().replace(tzinfo=None).floor("h")),
                'forecast': ((pd.Timestamp.utcnow().replace(tzinfo=None).ceil("d")-pd.Timestamp.utcnow().replace(tzinfo=None))).floor("h"),

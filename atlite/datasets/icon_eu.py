...
"""
Module for downloading and curating data from DWD ICON-EU dataset (via ODS).
...
dwd_url = "https://opendata.dwd.de/weather/nwp/icon-eu/grib/"

# ICON-EU model runs are available at fixed intervals: 00, 03, 06, 09, 12, 15, 18, 21 UTC
...
requirements = {
    "x": slice(-23.5, 62.5, 0.0625),  # Longitude range with resolution
    "y": slice(29.5, 70.5, 0.0625),  # Latitude range with resolution
    "offset": pd.Timedelta(hours=-18),  # Time offset for forecast initialization
    "forecast": pd.Timedelta(hours=120),  # Maximum forecast range
...
def _download(urls, tmpdir=None):
    ...
    for data_url in urls:
        logger.info(f"ICON-D2 data -> Processing file: {data_url}")
        ...
        if "generalVerticalLayer" in ds_temp.coords:
            ...
            ds_temp = ds_temp.expand_dims(
                dim=["valid_time", "generalVerticalLayer"]
            ).drop_vars(ds_coords_to_drop)
            ...
            if "step" in ds_temp.dims:
                ds_temp = ds_temp.swap_dims({"step": "valid_time"}).drop_vars(
                    ds_coords_to_drop
                )
            else:
                ds_temp = ds_temp.expand_dims(dim="valid_time").drop_vars(
                    ds_coords_to_drop
                )
...
def get_data(
    ...
    **creation_parameters :
    ...
    logger.info(f"Requesting data for feature {feature} for ICON-EU from open-dwd...")

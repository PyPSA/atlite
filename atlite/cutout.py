...
class Cutout:
    ...
    def profiles_at_points(
        ...
        **resource_kwargs,
        ...
        """
        Extract capacity factor time series at specific point coordinates.

        Computes the full per-cell capacity factor grid once (vectorized),
        then extracts the nearest grid cell for each location. This is
        much faster than creating a separate single-cell cutout per location,
        since it performs a single I/O pass and leverages vectorized
        computation across all grid cells.

        Parameters
        ----------
        locations : pd.DataFrame
            DataFrame with columns for name, x (longitude), and y (latitude).
        resource : str
            Resource function name to call on the cutout, e.g. ``"wind"``,
            ``"pv"``, ``"csp"``.
        name_col : str, default "name"
            Column in ``locations`` containing the location name (used as
            column names in the output).
        x_col : str, default "x"
            Column in ``locations`` containing longitude values.
        y_col : str, default "y"
            Column in ``locations`` containing latitude values.
        **resource_kwargs
            Keyword arguments passed to the resource function (e.g.
            ``turbine``, ``panel``, ``orientation``). The argument
            ``capacity_factor_timeseries=True`` is set automatically.

        Returns
        -------
        pd.DataFrame
            Capacity factor time series with ``DatetimeIndex`` and one
            column per location.
        ...
        ...     "y": [53.1, 51.7],
        ... })
        >>> wind = cutout.profiles_at_points(
        ...     locations, resource="wind", turbine="Vestas_V112_3MW"
        ... )
        >>> solar = cutout.profiles_at_points(
        ...     locations, resource="pv", panel="CSi",
        ...     orientation="latitude_optimal"
        ... )

        See Also
        --------
        convert_and_aggregate : Underlying conversion and aggregation logic.
        """
        func = getattr(self, resource)
        cf = func(capacity_factor_timeseries=True, **resource_kwargs)

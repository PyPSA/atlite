...
* Configuration can be accessed and changed within your code using
```
atlite.config.<config_variable>
```
* To list all `configuration_variables` currently in use
```
print(atlite.config.ATTRS)
```
* Create a `.yaml` file `atlite/config.yaml` in your home directory
    (`~/` on unix systems and usually `C:\\Users\\<Your Username>\\` on windows.

...
You can even submit your `Jupyter notebook`_ (``.ipynb``) directly as an example.
For contributing notebooks (and working with notebooks in `git` in general) 
we have compiled a workflow for you which we suggest you follow:
...
3. Provide a link to the documentation:
   Include a file ``foo.nblink`` located in ``doc/examples/foo.nblink`` with this content::
...
5. Commit your changes.
   If the precommit hook you installed above kicks in, confirm your decision ('y')
   or go back ('n') and delete the output of the notebook's cells.
6. Create a pull request for us to accept your example :)

For reasons and the details behind this workflow, see `this atlite/issues#38`_.

The support for the the notebook ``.ipynb`` format in our documentation is realised
via the extensions `nbsphinx`_ and `nbsphinx_link`_.
...
.. _this atlite/issues#38: https://github.com/PyPSA/atlite/issues/38
